...
workset_suffix_map = {
    ...
    BuiltInCategory.OST_LightingFixtures: "Furniture + Equipment", #padrão
...
lighting_suffix = "Placeholder LI"
...
# Tenta localizar o Placeholder LI
placeholder_li_ws = None
for workset in worksets:
    if workset.Name.strip().endswith(lighting_suffix):
        placeholder_li_ws = workset
        break
if not placeholder_li_ws:
    missing_worksets.append(lighting_suffix)

# Prefixos (em MAIÚSCULAS porque vamos comparar com name_upper)
SHAFTS_PREFIXES   = ("H+A_TEC", "H+A_MechanicalEquipment")
PLACEHOLDER_LI_PREFIXES = ("ATR_ID_LIG_B_PL", "ATR_ID_LIG_R_PL")
    ...
    for cat, default_ws in target_worksets.items():
        ...
        for el in elements:
            ...
            if param and not param.IsReadOnly:
               ...
               # Regras especiais para Lighting Placeholder
                if cat == BuiltInCategory.OST_LightingFixtures and placeholder_li_ws:
                    type_id = el.GetTypeId()
                    type_elem = doc.GetElement(type_id)
                    if type_elem:
                        try:
                            family = type_elem.Family
                            if family and family.Name.startswith(PLACEHOLDER_LI_PREFIXES):
                                target_ws = placeholder_li_ws
                        except:
                            pass
